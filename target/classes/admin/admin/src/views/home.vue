...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		...
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('baomingxinxi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{baomingxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>报名信息总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('ceshifankui','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji7" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{ceshifankuiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>测试反馈总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="baomingxinxiChart1" class="echarts1" v-if="isAuth('baomingxinxi','首页统计')"></div>
		<div id="ceshifankuiChart1" class="echarts2" v-if="isAuth('ceshifankui','首页统计')"></div>
		<div id="ceshifankuiChart2" class="echarts3" v-if="isAuth('ceshifankui','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getyonghuCount();
		this.getbaomingxinxiCount();
		this.baomingxinxiChat1();
		this.getceshifankuiCount();
		this.ceshifankuiChat1();
		this.ceshifankuiChat2();
	},
	methods:{
		// 词云
		...
		getbaomingxinxiCount() {
			this.$http({
				url: `baomingxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.baomingxinxiCount = data.data
				}
			})
		},
		baomingxinxiChat1() {
        ...
        var baomingxinxiChart1 = echarts.init(document.getElementById("baomingxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                baomingxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    baomingxinxiChart1.resize();
                };
            }
        });
      })
		...
		getceshifankuiCount() {
			this.$http({
				url: `ceshifankui/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.ceshifankuiCount = data.data
				}
			})
		},
		ceshifankuiChat1() {
        ...
        var ceshifankuiChart1 = echarts.init(document.getElementById("ceshifankuiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                ceshifankuiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ceshifankuiChart1.resize();
                };
            }
        });
      })
    ...
    ceshifankuiChat2() {
      this.$nextTick(()=>{
        ...
        var ceshifankuiChart2 = echarts.init(document.getElementById("ceshifankuiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                ceshifankuiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ceshifankuiChart2.resize();
                };
            }
        });
      })
