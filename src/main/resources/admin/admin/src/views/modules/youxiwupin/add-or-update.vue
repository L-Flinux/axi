	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="物品分类" prop="wupinfenlei">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.wupintupian" label="物品图片" prop="wupintupian">
					<file-upload
						tip="点击上传物品图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.wupintupian" label="物品图片" prop="wupintupian">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="物品介绍" prop="wupinjieshao">
					<el-input v-model="ruleForm.wupinjieshao" placeholder="物品介绍" clearable  :readonly="ro.wupinjieshao"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="物品介绍" prop="wupinjieshao">
					<el-input v-model="ruleForm.wupinjieshao" placeholder="物品介绍" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="物品用途" prop="wupinyongtu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.wupinyongtu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.wupinyongtu" label="物品用途" prop="wupinyongtu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.wupinyongtu"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wupinjieshao'){
							this.ruleForm.wupinjieshao = obj[o];
							this.ro.wupinjieshao = true;
							continue;
						}
						if(o=='wupinyongtu'){
							this.ruleForm.wupinyongtu = obj[o];
							this.ro.wupinyongtu = true;
							continue;
						}
						if(o=='price'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wupinyongtu = this.ruleForm.wupinyongtu.replace(reg,'../../../springboott9jh4c61/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `youxiwupin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.youxiwupinCrossAddOrUpdateFlag = false;
											this.parent.search();
